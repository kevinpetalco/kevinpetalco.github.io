...
ELSE 0
END as fines
   from cart where accountID='$row->accountID' order by cartID desc")or die(mysqli_error($con));
...
?>  						
                                            <tr>
                                               ...
                                               <td><?php if (!empty($rowdashboard->cartACCEPTED)) {
                                                echo $duedate= date('F d,Y H:i:s', strtotime($rowdashboard->cartACCEPTED . ' +7 day')); }else{ echo "Not Borrowed Yet";} ?></td>
                                               <td><?=$rowdashboard->fines;  ?></td>
